...
function App() {
  ...
  const marcarTomado = async (id) => {
    ...
    if (!error) {
      setSuplementos(
        suplementos.map((s) => (s.id === id ? { ...s, tomado: !s.tomado } : s)),
      );
    }
  };
  ...
  const buscarAlimento = () => {
    ...
    setCargando(true);
    fetch(
      `https://world.openfoodfacts.org/cgi/search.pl?search_terms=${busqueda}&json=true&page_size=5`,
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className="max-w-xl mx-auto mt-6 px-4 font-sans pb-10">
      ...
      <p className="text-center text-gray-500 mb-6">
        ✅ {suplementos.filter((s) => s.tomado).length} de {suplementos.length}{" "}
        tomados
      </p>
      ...
      <ul style={{ listStyle: "none", padding: 0 }}>
        {suplementos.map((s) => (
          <li
            key={s.id}
            className={`flex justify-between items-center p-4 mb-3 rounded-xl border transition-all ${s.tomado ? "bg-green-50 border-green-200" : "bg-gray-50 border-gray-200"}`}
          >
            <span
              className={
                s.tomado ? "line-through text-gray-400" : "text-gray-800"
              }
            >
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Calorías: {producto.nutriments?.energy_value || "N/A"} kcal
                </p>
              </div>
              <button
                onClick={() =>
